...
<body>
    ...
    <div class="links-container">
        ...
        <form method="get" action="http://localhost/Project/08_library.php">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["search"])) {
        // Redirect to the library page with the search input as a query parameter
        $search_query = urlencode($_GET["search"]);
        header("Location: http://localhost/Project/08_library.php?search={$search_query}");
        exit();
    }
    ...
    if (isset($_GET['id'])) {
        ...
        if ($result_playlist->num_rows > 0) {
            ...
            $stmt_songs = $conn->prepare("SELECT ps.position, s.title, s.genre, s.release_date, s.audio_url FROM playlist_song ps 
                JOIN song s ON ps.song_id = s.song_id WHERE ps.playlist_id = ? ORDER BY ps.position");
            ...
            if ($result_songs->num_rows > 0) {
                ...
                while ($row_song = $result_songs->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>
                                <audio controls>
                                    <source src=\"{$row_song['audio_url']}\" type=\"audio/mp3\">
